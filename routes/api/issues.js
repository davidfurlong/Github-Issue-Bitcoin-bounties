...
exports.claimBountyCallback = function(req, res){
	...
	Request.post("https://github.com/login/oauth/access_token", options, function(err, response, body){
		...
		Issue.find(issueId).then(function(issue){
			...
			url = "https://api.github.com/repo/" + issue.user + "/" + issue.repo + "/issue/" + issueNumber;
			console.log(url)
					...
					sequelize.transaction(function(t) {
						...
						{transaction:t}
							...
							Issue.find(qr2.IssueId, {transaction:t}).then(function(qr3){
								...
								commentOnGithubIssue([qr3.user,qr3.repo,qr3.uri.substring(qr3.uri.lastIndexOf('/') + 1)], "A new bounty of " + tra.amount + "BTC has been added for this issue! View all bounties at: http://davidfurlong.github.io/Spur/issue.html?id=" + qr3.id, function(a,b) { })
